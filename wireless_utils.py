...
class ComplexWirelessChannel(nn.Module):
    def __init__(self, snr_dB=15.0, fading='none', rician_k=4.0):
        super().__init__()
        self.snr_dB = snr_dB
        self.fading = fading.lower()
    ...
    def power_norm_batchwise(self, signal, power=1.0):
        ...
        signal_power = torch.sum((signal[:,:,0]**2 + signal[:,:,1]**2), dim=1) / num_symbols
        signal = signal * math.sqrt(power) / torch.sqrt(signal_power.unsqueeze(1).unsqueeze(2))
        return signal
    ...
    def apply_fading(self, signal, fading_type, rician_k=4.0):
        ...
        if fading_type == 'rayleigh':
            ...
            mean = math.sqrt(rician_k / (2*(rician_k + 1)))
            std = math.sqrt(1 / (2*(rician_k + 1)))
    ...
    def forward(self, x, snr=None, fading=None, rician_k=None, modal=False, noisy_k=1.0):
        """
        x: [batch, seq_len, dim] real-valued input
        """
        rician_k = self.rician_k if rician_k is None else rician_k
        ...
        # print(f'original x norm: {torch.linalg.norm(x_flat)}')
        ...
        # print(f'power norm x: {torch.linalg.norm(x_complex)}')
        ...
        if padded:
            y_flat = y_flat[:, :-1]
            # y_noisy_flat = y_noisy.view(batch_size, -1)[:, :-1]
            # x_complex_flat = x_complex.view(batch_size, -1)[:, :-1]

        # print(f'equalized y norm: {torch.linalg.norm(y_flat)}')
        # print(f'distance between x and y: {torch.linalg.norm(x_flat - y_flat)}')

        y_out = y_flat.view(original_shape)

        # return y_out, x_complex_flat.view(original_shape), y_noisy_flat.view(original_shape)
        return y_out
