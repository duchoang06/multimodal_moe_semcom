    ...
    def forward(
        ...
        if self.hr > 0:
            ...
            if modality_ids is not None:
                ...
                loss_img = attn_load_balance(gates, mask, modality_mask=(flat_mod_ids == 0), pad_mask=flat_pad)

                loss_txt = attn_load_balance(gates, mask, modality_mask=(flat_mod_ids == 1), pad_mask=flat_pad)

                aux_loss['lb'] = (loss_img + loss_txt) / 2.0

            else:
                all_valid = torch.ones(B * T, device=x.device, dtype=torch.bool)
                aux_loss['lb'] = attn_load_balance(gates, mask, modality_mask=all_valid, pad_mask=flat_pad)
...
class MoEFFN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.linear_proj = nn.Linear(cfg.d_model * 2, cfg.d_model)  # Project concatenated features back to d_model

    def forward(self, x: torch.Tensor, modality_ids: torch.Tensor, task_emb: torch.Tensor):
        # Extract shared/task-dependent features via MToE

        # data_tokens = x[:, 1:, :]

        mtoe_out, cmi_loss = self.mtoe(x, modality_ids, task_emb)
        
        # new_task_token = mtoe_out.mean(dim=1, keepdim=True)

        # Extract modality-specific features via MSoE
        msoe_out, msoe_loss_dict = self.msoe(x, modality_ids)
        #TO-DO: maybe load balancing for MSoE blocks
        
        # 3. Concat task and modality-specific embeddings
        y = torch.cat([mtoe_out, msoe_out], dim=-1)  # [B, T, 2*d_model]
        y = self.linear_proj(y)  # Project back to d_model
...
class MToE(nn.Module):
    def __init__(self, cfg):
        ...
        # NEW: Projects your clean, early task embedding into an expert bias
        self.task_proj = nn.Linear(cfg.d_model, cfg.mtoe_n_experts, bias=False)
        
        # Initialize to zero so the router starts neutral and learns the shift
        nn.init.zeros_(self.task_proj.weight)

    def route(self, x: torch.Tensor, early_task_embedding: torch.Tensor) -> torch.Tensor:
        """
        x: [B, T, d_model] - The token representations after Transformer layers.
        early_task_embedding: [B, d_model] - The clean task embedding from the start of the network.
        """
        # 1. Base routing logits from token features: [B, T, n_experts]
        logits = self.router(x) 
        
        # 2. Map the clean task embedding to expert biases: [B, n_experts]
        task_bias = self.task_proj(early_task_embedding)
        
        # 3. Expand dimensions so it can be added to every token in the sequence
        # task_bias becomes [B, 1, n_experts]
        task_bias = task_bias.unsqueeze(1)
        
        # 4. Combine them
        logits = logits + task_bias
    ...
    def compute_cmi_loss(self, probs: torch.Tensor, modality_ids: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
        """
        Calculates Mutual Information I(M; E). Minimize this to encourage modality-agnostic task experts.
        """
        B, T, E = probs.shape
        probs_flat = probs.view(-1, E)
        mod_flat = modality_ids.view(-1)
        
        unique_mods = torch.unique(mod_flat)
        num_present_mods = len(unique_mods)
        
        # If only one modality is present in the batch, MI is 0
        if num_present_mods <= 1:
            return torch.tensor(0.0, device=probs.device, requires_grad=True)
            
        # 1. P(E | M) - Use MEAN to eliminate sequence length bias
        p_e_given_m = torch.zeros(num_present_mods, E, device=probs.device)
        for idx, mod_id in enumerate(unique_mods):
            mask = (mod_flat == mod_id)
            p_e_given_m[idx] = probs_flat[mask].mean(dim=0)
            
        # 2. P(M) - Assume uniform prior to balance modalities natively
        p_m = torch.ones(num_present_mods, 1, device=probs.device) / num_present_mods
        
        # 3. Compute Joint P(M, E) and Marginal P(E)
        joint_probs = p_e_given_m * p_m              # Shape: [M, E]
        ...
        # Return positive MI to MINIMIZE it (encouraging modality independence)
        return mi

    def forward(self, x, modality_ids, task_emb):
        B, T, D = x.shape
        
        # 1. Routing
        logits = self.route(x, task_emb)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        # x: [B, T+1, D]
        a, aux_attn_dict = self.attn(self.ln1(x), attn_mask=attn_mask, modality_ids=modality_ids, pad_mask=pad_mask)
        x = x + self.drop1(a)

        aux_loss_block['attn'] = aux_attn_dict

        f, aux_ffn_dict = self.ffn(self.ln2(x), modality_ids=modality_ids, task_emb=task_emb)
...
class MultiModalMultiTaskMoHMoE(nn.Module):
    ...
    def forward(
        ...
        if vision_tokens is not None:
            ...
            parts.append(xv)
            modality_ids.append(torch.full((B, Tv), 0, device=device, dtype=torch.long)) #append [B,Tv] of 0s
            pad_masks.append(torch.ones((B, Tv), device=device, dtype=torch.long))
        ...
        if text_tokens is not None:
            ...
            parts.append(xt)
            modality_ids.append(torch.full((B, Tt), 1, device=device, dtype=torch.long))

            text_mask = (text_tokens != 0).long()
            pad_masks.append(text_mask)
        ...
        pad_mask = torch.cat(pad_masks, dim=1)           # [B,T] - 1 for valid, 0 for pad
        ...
        modality_emb = self.modality_emb(mod)
        x = x + modality_emb + self.pos_emb(pos)

        # Inject task embedding as an additive bias on all tokens (simple and effective)
        task_emb = self.task_emb(task_id)
        x = x + task_emb.unsqueeze(1)  # [B,1,D] broadcast to [B,T,D]
        x = self.dropout(x)
        ...
        for blk in self.blocks:
            x, aux_blk = blk(x, attn_mask=attn_mask, modality_ids=mod, task_ids=task_id, pad_mask=pad_mask, task_emb=task_emb)
